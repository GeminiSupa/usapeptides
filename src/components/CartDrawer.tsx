        ...
        <div className="w-screen max-w-md bg-brand-card border-l border-brand-border flex flex-col">
          ...
          <div className="p-4 border-b border-brand-border bg-brand-darker flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-base font-bold text-white">Your Research Cart</h2>
              <span className="text-xs bg-brand-dark text-cyan-400 px-2 py-0.5 rounded-full border border-brand-border font-mono">
            ...
            <div className="flex items-center justify-between text-xs mb-1.5">
              <span className="flex items-center gap-1.5 text-gray-300">
                ...
                {hasFreeShipping ? (
                  ...
                  <span>Add <strong className="text-cyan-300">${amountNeededForFreeShipping.toFixed(2)}</strong> more for Free Shipping</span>
                )}
              </span>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {cart.length === 0 ? (
              <div className="text-center py-16 space-y-4">
                <div className="w-16 h-16 rounded-full bg-brand-dark mx-auto flex items-center justify-center text-gray-600 border border-brand-border">
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-start justify-between gap-2">
                      <Link
                        ...
                        onClick={() => setIsCartOpen(false)}
                        className="text-xs font-bold text-white hover:text-cyan-400 line-clamp-1"
                      >
                        {item.product.name}
                      </Link>
                      <button
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <div className="flex items-center border border-brand-border rounded-lg bg-brand-darker overflow-hidden">
                        <button
                          ...
                          className="px-2 py-1 text-gray-400 hover:text-white hover:bg-brand-card"
                        >
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="px-2 py-1 text-xs text-white font-mono font-bold">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() => updateQuantity(item.product.id, item.quantity + 1)}
          ...
          {cart.length > 0 && (
            <div className="p-4 border-t border-brand-border bg-brand-darker space-y-3">
              ...
              {bulkDiscountSavings > 0 && (
                <div className="flex items-center justify-between text-xs text-emerald-400 bg-emerald-950/40 border border-emerald-500/30 px-3 py-1.5 rounded-lg">
              ...
              {/* Coupon discount */}
              {couponDiscount > 0 && (
                <div className="flex items-center justify-between text-xs text-cyan-400 bg-cyan-950/40 border border-cyan-500/30 px-3 py-1.5 rounded-lg">
              ...
              <div className="space-y-1.5 text-xs text-gray-400 pt-1">
                ...
                <div className="flex justify-between text-sm font-extrabold text-white pt-2 border-t border-brand-border">
              ...
              <div className="space-y-2 pt-2">
                ...
                <Link
                  href="/cart"
                  onClick={() => setIsCartOpen(false)}
                  className="w-full py-2.5 px-4 rounded-xl bg-brand-card hover:bg-brand-cardHover border border-brand-border text-gray-300 hover:text-white text-xs font-semibold transition-colors text-center block"
                >
